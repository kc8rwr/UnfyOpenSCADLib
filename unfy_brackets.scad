...
module unf_simple_bracket(top_bolt_size = "M4",
  ...
  let(top_washer_v = unf_wsh_v(top_bolt_size),
      ...
      top_length = top_bolt_position + (top_washer_d / 2) + wall,
      bottom_length = bottom_bolt_position + (bottom_washer_d / 2) + wall, //2*wall = wall taken up by width of top_side + wall-widthed outer edge
      round_edge = wall / 4) {
    color(body_color){
	...
	difference(){
	  ...
	  translate([width/2, bottom_length-bottom_bolt_position, 0]){
	    translate([0, 0, wall+$over]){
	      rotate([0, 180, 0]){
		unf_wsh(size=bottom_washer_v, ext=$over);
	      }
	    }
	    translate([0, 0, -$over]){
	...
	rotate([-90, 0, 0]){
	  translate([0, -(top_length), 0]){
		...
		translate([2*wall, top_length-wall, 0]){
	      ...
	      translate([width/2, top_length-top_bolt_position, $over]){
		translate([0, 0, 0]){
		  unf_wsh(size=top_washer_v, ext=wall/2);
		}
		translate([0, 0, -$over]){
