...
module unf_PopinCustom_Negative(size=[10, 5], ridge=[2, 1], wall=2){
  translate([0, 0, -$over]){
    cube([size.x+(2*ridge.x), size.y+(2*ridge.y), wall + (2*$over)]);
  }
}

module unf_PopinCustom_Positive(size=[10, 5], catch_thickness=1.2, ridge=[2,1], wall=2, support_skin=0){
...
module unf_Popin_Positive(device, wall=2, support_skin=0){
  let(device = unf_stToLower(unf_stCutAfter(device, "|"))){
    if ("rleil_r2" == device){
      unf_rleil_rl2_popin(wall=wall, support_skin=support_skin);
    } else if ("yb4835va" == device){
      unf_yb4835va_popin(wall=wall, support_skin=support_skin);
    }
  }
}
...
module unf_Popin(location=[0, 0, 0], rotation=0, device="rleil_r2", wall=2, support_skin=0){
  difference(){
    children();
    translate(location){
      rotate(rotation){
  ...
  translate(location){
    rotate(rotation){
      unf_Popin_Positive(device=device, wall=wall, support_skin=support_skin);
    }
  }
...
module unf_rleil_rl2_popin(wall=2, support_skin=0){
  unf_PopinCustom_Positive(size=[30.2, 22], catch_thickness=1.75, ridge=[1.5, 0], wall=wall, support_skin=support_skin);
}
...
//YB4835VA Volt/Amp meter
module unf_yb4835va_popin(wall=2, support_skin=0){
  unf_PopinCustom_Positive(size=[66, 37], wall=wall, ridge=[3, 0], support_skin=support_skin);
}
...
unf_Popin(location=[margin, margin, 0], rotation=0, device=device, wall=wall, support_skin=support_skin){
