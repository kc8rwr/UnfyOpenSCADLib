...
// Description: (key, vector[key, value]) - Looks up a value from a table, like the built-in lookup function but interpolate even beyond the bounds of the vector
// Usage: lookup(key, values)
// Arguments:
//   key = the lookup key
//   values = the value table to look within
function unf_lookup(key, values) = 0 == len(values) ? undef : (
  1 == len(values) ? values[0][1] : (
    key < values[0][0] ? values[0][1]-(((values[1][1]-values[0][1])/(values[1][0]-values[0][0])*(values[0][0]-key))) : (
      key <= values[len(values)-1][0] ? lookup(key, values) : (
	values[len(values)-1][1]+((((values[len(values)-1][0]-values[len(values)-2][0])/(values[len(values)-1][1]-values[len(values)-2][1]))*(key-values[len(values)-1][0])))
      )
    )
  )
);

// Function: unf_stCutAfter
// Synopsis: Return a string with everything after the beginning of the first occurance of a substring cut off.
// Description: Find the first occurrance of cutoff within in and return a string that consists of in, only up to that point with the rest cut off.
// Usage: unf_stCutAfter(string in, string cutoff)
// Arguments:
//   in = the input string
...
// Description: Return a copy of a string with any whitespace on the front or back trimmed off.
// Usage: unf_strTrim(in)
// Arguments:
//   in = the input string
...
// Description: Convert a vector to a string.
// Usage: unf_vToS(in)
// Arguments:
//   in = input vector
...
// Function: unf_stToLower
// Synopsis: Convert a string to lowercase.
// Description: Take an input string and return a copy where every uppercase character has been converted to it's lowercase equivalent.
// Usage: unf_stToLower(in)
// Arguments:
//   in = the input string
...
// Function: unf_stToUpper
// Synopsis: Convert a string to uppercase.
// Description: Take an input string and return a copy where every lowercase character has been converted to it's uppercase equivalent.
// Usage: unf_stToUpper(in)
// Arguments:
//   in = the input string
...
// Function: unf_chToLower
// Synopsis: Convert a character to lowercase
// Description: Accepts a single character and if it is uppercase then return it's lowercase equivalent otherwise return the original input.
// Usage: unf_chToLower(in)
// Arguments:
//   in = the input character
function unf_chToLower(in) = chr(unf_ordToLower(ord(in)));

// Function: unf_chToUpper
// Synopsis: Convert a character to uppercase
// Description: Accepts a single character and if it is lowercase then return it's uppercase equivalent otherwise return the original input.
// Usage: unf_chToUpper(in)
// Arguments:
//   in = the input character
...
// Function: unf_ordToLower
// Synopsis: Convert a character represented as a unicode number to lowercase
// Description: Converts a number representing a unicode character to the number representing it's lowercase version
// Usage: unf_ordToLower(in)
// Arguments:
//   in = the input number
...
// Function: unf_ordToUpper
// Synopsis: Convert a character represented as a unicode number to uppercase
// Description: Converts a number representing a unicode character to the number representing it's uppercase version
// Usage: unf_ordToLower(in)
// Arguments:
//   in = the input number
...
// Function: unf_matrix_sort
// Synopsis: Sort a 2-dimensional matrix
// Description: Sorts a vector of vectors by the values in one of the columns of the inner vectors. *adapted from quicksort example OpenSCAD manual https://en.wikibooks.org/wiki/OpenSCAD_User_Manual/The_OpenSCAD_Language#Sorting_a_vector*
// Usage: unf_matrix(arr, sort_col_num)
// Arguments:
