...
// Arguments
//   size = fastener size string or vector representing an unfy_nut (hex nut)
function unf_nut_diameter(size) = is_list(size) ? size[2] : (
	"m" == size[0] || "M" == size[0] ? (
		unf_round(place=-3,
					 unf_lookup(unf_stToNum(unf_sub(size, 1)),
									[[0, 0],
									 [1.6, 3.7],
									 [2, 4.62],
									 [2.5, 5.77],
		...
		"\"" == in[len(size)-1] ? (
			unf_round(place=-3,
						 num=unf_lookup(25.4*unf_stToNum(unf_sub(size, 0, len(size)-1)),
											 [[0, 0],
...
// Figure(Spin;VPD=30; VPT=[0, 0, -2];  NoAxes): note - the ext area is semi-transparent.
//   $fn = 36;
//   $over = 0.1;
//   $wall=2;
//   use <unfy_fasteners.scad>;
...
// Figure(Spin;VPD=40; VPT=[0, 0, 2]; NoAxes): note - the ext area is semi-transparent.
//   $fn = 36;
//   $over = 0.1;
//   $wall=2;
//   use <unfy_fasteners.scad>;
